...
void ValuePrint(FILE *stream, Value value)
{
    switch (value.type)
    {
    case VAL_NUMBER:
        OutputWrite(stream, "%g", AS_NUMBER(value));
        break;
    case VAL_BOOL:
        OutputWrite(stream, "%s", AS_BOOL(value) ? "@true" : "@false");
        break;
    case VAL_RESULT:
        OutputWrite(stream, "%s", AS_SUCCESS(value) ? "@success" : "@error");
        break;
    case VAL_OBJ:
    case IJO_INTERNAL_STRING:
        ObjectPrint(stream, value);
        break;
    case IJO_NOOP:
        break;
    }
