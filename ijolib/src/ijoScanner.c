...
Token constOrKeyword(Scanner *scanner)
{
  ...
  {
  case '{':
    return makeComplexeToken(scanner, TOKEN_STRUCT, identifierStart, identifierLength);
  case '[':
    return makeComplexeToken(scanner, TOKEN_ARRAY, identifierStart, identifierLength);
  case '<':
    return makeComplexeToken(scanner, TOKEN_MAP, identifierStart, identifierLength);
  case '|':
    return makeComplexeToken(scanner, TOKEN_ENUM, identifierStart, identifierLength);
  case '%':
    return makeComplexeToken(scanner, TOKEN_MODULE, identifierStart, identifierLength);
  case '(':
    return makeComplexeToken(scanner, TOKEN_FUNC, identifierStart, identifierLength);
  case '@':
    return makeComplexeToken(scanner, TOKEN_ASSERT, identifierStart, identifierLength);
  case '=':
...
TokType identifierType(Scanner *scanner)
{
  ...
  case '@':
    if (scanner->current - scanner->start > 1)
    {
      ...
      {
      case 't':
        return checkKeyword(scanner, 2, 3, "rue", TOKEN_TRUE);
      case 'f':
